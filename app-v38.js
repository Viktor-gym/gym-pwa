...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function viewWorkout(){
    const el = document.createElement("div");
    startWorkoutIfNeeded();
    const hasWorkoutItems = (workoutSession.items||[]).length > 0;
      ...
      el.appendChild(card(`
        <div class="row" style="justify-content:space-between; align-items:flex-start">
          ...
          <div class="row workoutQuickActions">
            ...
            <button class="btn" id="addTemplateToW">＋ ${state.lang==="en"?"Add complex":"Додати комплекс"}</button>
          </div>
        </div>
      `));
    ...
    setTimeout(()=>{
      ...
      const addBottomBtn = $("#addExToWBottom");
    ...
    modal.innerHTML = `
      <div class="detailHeader">
        <div class="detailTitle">
          <h2>${t("addExercise")}</h2>
          <div class="sub">${state.lang==="en"?"Select several exercises, then confirm.":"Обери кілька вправ, потім підтвердь додавання."}</div>
        </div>
      ...
      <div class="pickerFooter">
        ...
        <button class="btn primary" id="pickConfirm">${state.lang==="en"?"Add selected":"Додати вибрані"}</button>
      </div>
    `;
        ...
        return `
          <div class="itemRow ${picked?"selectedPick":""}">
            ...
            <div class="row">
              ...
              <button class="btn ${picked?"primary":""}" data-toggle-pick="${ex.id}">${picked?(state.lang==="en"?"Selected":"Вибрано"):"＋"}</button>
            </div>
          </div>
        `;
      }).join("");
      const counter = modal.querySelector("#pickSelectedCount");
      if(counter) counter.textContent = state.lang==="en" ? `${selected.size} selected` : `Вибрано: ${selected.size}`;
  ...
  function openTemplatePickerForWorkout(){
    ...
    if(!templates.length){
      alert(state.lang==="en"?"Create a complex template first.":"Спочатку створи шаблон комплексу.");
      return;
    }
    const overlay=document.createElement("div");
    overlay.className="modeOverlay";
  ...
  function latestExercisePerformance(exerciseId){
    ...
    return {
      ...
      sets:(latest.item.sets||[]).map(set=>({...set,_restTriggered:true}))
    };
  }
    ...
    workoutSession={
      ...
      items:(workout.items||[])
        ...
        .map(item=>({
          id:uid(),
          exerciseId:item.exerciseId,
          sets:(item.sets||[]).map(set=>({...set,_restTriggered:true}))
        }))
    };
    resetRestTimer();
  ...
  function addExercisesToSession(exerciseIds,options={}){
    ...
    unique.forEach(id=>{
      ...
      const latestSets=latest?.sets?.length ? latest.sets.map(set=>({...set,_restTriggered:true})) : [];
      if(!existing){
  ...
  function plannedWorkoutRowMarkup(plan){
    ...
    const timeText=plan.time ? ` · ${escapeHtml(plan.time)}` : "";
    return `<article class="plannedWorkoutRow">
      <div class="plannedDate"><span>${fmtDate(plan.date)}</span>${plan.time?`<small>${escapeHtml(plan.time)}</small>`:""}<strong>${items.length}</strong><small>${state.lang==="en"?"ex.":"впр."}</small></div>
      <div class="plannedMain"><strong>${escapeHtml(plan.title)}</strong><span>${fmtDate(plan.date)}${timeText} · ${escapeHtml(names || (state.lang==="en"?"No exercises":"Немає вправ"))}${items.length>4?` · +${items.length-4}`:""}</span></div>
    ...
    return `
      <section class="planningSuite">
        <div class="planningSide planningTopList">
          <div class="planningSubhead"><strong>${state.lang==="en"?"Planned workouts":"Заплановані тренування"}</strong><button class="btn" id="newPlanBtn">＋ ${state.lang==="en"?"Plan":"План"}</button></div>
          ...
          <div class="planningSubhead" style="margin-top:12px"><strong>${state.lang==="en"?"Complex templates":"Шаблони комплексів"}</strong><button class="btn" id="newTemplateBtn">＋ ${state.lang==="en"?"Template":"Шаблон"}</button></div>
  ...
  function renderExList(){
    ...
    box.innerHTML = list.map(ex=>{
      ...
      return `
        <div class="itemRow" data-editex="${ex.id}" role="button" tabindex="0">
          ...
          <div class="row">
            <button class="btn" data-fav="${ex.id}">${isFav(ex.id)?"⭐":"☆"}</button>
            <button class="btn primary" data-editex-btn="${ex.id}">${state.lang==="en"?"Edit":"Редагувати"}</button>
    ...
    box.querySelectorAll("[data-editex-btn]").forEach(b=>{
      b.onclick = (e)=>{
        e.stopPropagation();
        const ex = state.exercises.find(item=>item.id===b.getAttribute("data-editex-btn"));
        if (ex) openAddExerciseModal(ex);
      };
    });
    ...
    const steps=[
      {
        ...
        copy:en
          ? "Open Exercises and tap + to create an item in your library. Tap an exercise card or Edit to rename it, move it to another category, or correct tracking: weight + reps, reps only, duration, or distance + time."
          : "Відкрий «Вправи» й натисни +, щоб створити позицію в бібліотеці. Натисни на картку вправи або «Редагувати», щоб перейменувати її, перенести в іншу категорію або виправити тип обліку: вага + повтори, лише повтори, час або дистанція + час.",
        ...
        mini:`<div class="miniTitle">＋ ${en?"New exercise":"Нова вправа"}</div><div class="miniField">${en?"Pull-ups":"Підтягування"}</div><div class="miniRow"><div class="miniField">${en?"Back":"Спина"}</div><div class="miniField">${en?"Edit":"Редагувати"}</div></div><div class="miniButton" style="margin-top:6px;text-align:center">${en?"Save":"Зберегти"}</div>`
      },
      {
  ...
  function usageGuideMarkup(){
    ...
    const userSteps=[
      ...
      },
      {
        ...
        copy:en
          ?"Open Exercises to maintain your exercise library. Tap + to create a new exercise. Tap an existing exercise card or Edit to rename it, move it to another muscle group, or correct the tracking type: weight + reps, reps only, duration, or distance + time."
          :"Відкрий «Вправи», щоб вести бібліотеку вправ. Натисни +, щоб створити нову вправу. Натисни на наявну картку або «Редагувати», щоб перейменувати її, перенести в іншу групу м’язів або виправити тип обліку: вага + повтори, лише повтори, тривалість або дистанція + час.",
        ...
        mini:`<div class="miniTitle">＋ ${en?"Exercise":"Вправа"}</div><div class="miniField">${en?"Pull-ups":"Підтягування"}</div><div class="miniRow"><div class="miniField">${en?"Back":"Спина"}</div><div class="miniField">${en?"Edit":"Редагувати"}</div></div>`
      },
      {
