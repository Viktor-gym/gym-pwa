...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function activateWorkoutFromHistory(workoutId){
    ...
    workoutSession={
      ...
      title:workout.title || (state.lang==="en"?"Workout":"Тренування"),
      workoutId:null,
      planId:null,
      items:(workout.items||[])
  ...
  function renderExList(){
    ...
    box.querySelectorAll("[data-addw]").forEach(b=>{
      b.onclick = ()=>{
        ...
        const copyLatest=hasLatestPerformance([id]) && confirm(state.lang==="en"
          ? "Add sets and weights from the latest workout for this exercise?"
          : "Додати підходи й ваги з останнього заняття для цієї вправи?");
        addExercisesToSession([id],{copyLatest});
        render();
      };
    });
  }
